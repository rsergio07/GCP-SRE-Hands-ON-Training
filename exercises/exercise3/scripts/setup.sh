...
# Check if container images exist in Artifact Registry
check_images() {
    print_status "Checking for container images..."
    
    # Check if images exist in Artifact Registry
    if gcloud artifacts docker images list us-central1-docker.pkg.dev/$PROJECT_ID/sre-demo-app --limit=1 &>/dev/null; then
        print_success "Container images found in registry"
        # List the tags for confirmation
        gcloud artifacts docker tags list us-central1-docker.pkg.dev/$PROJECT_ID/sre-demo-app --limit=5
    else
        print_warning "No container images found. Make sure to complete Exercise 2 first"
        print_status "You can check images with: gcloud artifacts docker images list us-central1-docker.pkg.dev/$PROJECT_ID/sre-demo-app"
        exit 1
    fi
}
...
update_manifests() {
    ...
    # Use sed to replace the placeholder with the actual project ID in each YAML file
    # Use a backup extension that won't overwrite existing files
    
    if [ -f "k8s/deployment.yaml" ]; then
        sed -i.bak "s/PROJECT_ID/$PROJECT_ID/g" k8s/deployment.yaml
        sed -i.bak "s/PROJECT_ID/$PROJECT_ID/g" k8s/configmap.yaml
        sed -i.bak "s/PROJECT_ID/$PROJECT_ID/g" k8s/service.yaml
        sed -i.bak "s/PROJECT_ID/$PROJECT_ID/g" k8s/hpa.yaml
        print_success "Manifest files updated"
    else
        print_warning "k8s/deployment.yaml not found. Manifest files not updated."
    fi
}
